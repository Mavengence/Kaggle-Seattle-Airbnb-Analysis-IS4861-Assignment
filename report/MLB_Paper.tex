...
This working paper can not explain the whole math behind the Elastic Net Classiefer, because it is to complicated. A general summary would be that the elastic net is a convex sum of ridge and lasso penalties, so the objective function for a Gaussian error model looks like:
\begin{equation}
...
The most obvious estimation is that our network's design (figure \ref{lstm}) is not appropriate for this kind of prediction. Even the linear regression (figure \ref{linear_regression}) scored way better than the MAE of the LSTM network (figure \ref{nn}). \\
The bad result (figure \ref{nn}) could be additionally explained by the missing features as already explained \cite{RN1}.
